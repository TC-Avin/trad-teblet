...
const MyAccountTable = (props) => {
    ...
    return (
    <div className='lg:px-12 px-3 pt-2 lg:pb-8 pb-0'>
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg ">
            <table class="w-full text-sm text-center text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-white uppercase bg-[#3491FF] ">
                    <tr>
                    <th scope="col" class="px-6 py-3 text-center md:text-base text-xs font-bold"></th>
                        <th scope="col" class="px-6 py-3 text-center md:text-base text-xs font-bold">BillNo</th>
                        <th scope="col" class="px-6 py-3 text-center md:text-base text-xs font-bold">Date</th>
                        <th scope="col" class="px-6 py-3 text-center md:text-base text-xs font-bold">Amount</th>
                        <th scope="col" class="px-6 py-3 text-center md:text-base text-xs font-bold">Received</th>
                        <th scope="col" class="px-6 py-3 text-center md:text-base text-xs font-bold">TDS</th>
                        <th scope="col" class="px-6 py-3 text-center md:text-base text-xs font-bold">Balance</th>
                        <th scope="col" class="px-6 py-3 text-center md:text-base text-xs font-bold">Action</th>
                    </tr>
                </thead>
                <tbody>
                    {rows.map((row,i)=>(
                  ...
                  <div>
                    ...
                    />
                            ...
                            <th class='py-4 text-[#3491FF] md:text-lg text-xs cursor-pointer'onClick={()=>{props.setinvoice(true)}}>{row.BillNo}</th>
                            <th class='py-4 text-[#3D3D3D] md:text-lg text-xs font-medium'>{row.Date}</th>
                            <th class='py-4 text-[#3D3D3D] md:text-lg text-xs font-medium'>{row.Amount}</th>
                            <th class='py-4 text-[#3D3D3D] md:text-lg text-xs font-medium'>{row.Received}</th>
                            <th class='py-4 text-[#3D3D3D] md:text-lg text-xs font-medium'>{row.TDS}</th>
                            <th class='py-4 text-[#3D3D3D] md:text-lg text-xs font-medium'>{row.Balance}</th>
        ...
        <div className='flex justify-between md:my-8 my-6'>
                <a href='#' className='text-[#3491FF] underline lg:text-lg text-base underline-offset-1'>Last Updated On 03-06-2022 at 10:38</a>
                <a href='#' className='text-[#3491FF] underline lg:text-lg text-base underline-offset-1'>Complience Check for Section 206AB & 206CCA</a>
            </div>
    </div>
  )
}
